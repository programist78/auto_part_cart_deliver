...
const SignUp: React.FC<{ mode: 'seller' | 'buyer' }> = ({ mode }) => {
...
const BuyerForm: React.FC = () => {
    ...
    const { handleBlur, handleChange, handleSubmit, values, errors, touched } = useFormik({
        ...
        onSubmit: async ({ emailOrPhore:email, name:fullname, password, rePassword:confirm_password }) => {
            axios.post('/api/auth/register', { email, confirm_password, fullname, password, role: 'USER' }).then(({ data }) => {
    ...
    return <div className={c.main}>
        <form className={c.form} onSubmit={handleSubmit}>
            <h1 className={c.title}>Create Buyer Account</h1>
            ...
            <div className={c.inp_con}>
                <label className={c.label} htmlFor='emailOrPhore'>Email or Phone Number</label>
                <input className={errors.emailOrPhore && touched.emailOrPhore ? c.inp_error : c.inp} id='emailOrPhore' name='emailOrPhore' placeholder='hello@yachtease.co' onChange={handleChange} onBlur={handleBlur} value={values.emailOrPhore} />
                {errors.emailOrPhore && touched.emailOrPhore && <span className={c.error}>{errors.emailOrPhore}</span>}
            </div>

            <div className={c.inp_con}>
                <label className={c.label} htmlFor='password'>Password</label>
                <input className={errors.password && touched.password ? c.inp_error : c.inp} id='password' name='password' type='password' placeholder='Password' onChange={handleChange} onBlur={handleBlur} value={values.password} />
                {errors.password && touched.password && <span className={c.error}>{errors.password}</span>}
            </div>

            <div className={c.inp_con}>
                <label className={c.label} htmlFor='rePassword'>Re-enter Password</label>
                <input className={errors.rePassword && touched.rePassword ? c.inp_error : c.inp} style={{ marginBottom: errors.rePassword && touched.rePassword ? 45 : 10 }} id='rePassword' name='rePassword' type='password' placeholder='Re-enter Password' onChange={handleChange} onBlur={handleBlur} value={values.rePassword} />
                {errors.rePassword && touched.rePassword && <span className={c.error}>{errors.rePassword}</span>}
            </div>
            ...
            <div className={c.new}>
                <span>By creating an account, you agree with Conditions of Use</span>
            </div>
        </form>
    </div>
}
...
const SellerForm: React.FC = () => {
    ...
    const { handleBlur, handleChange, handleSubmit, values, errors, touched } = useFormik({
        ...
        onSubmit: async ({ address, emailOrPhore: email, name:fullname, password, rePassword:confirm_password }) => {
            axios.post('/api/auth/register', { email, confirm_password, fullname, password, address, role: 'BUSINESS' }).then(({ data }) => {
                setError(null)
                setUserData(data.user)
                Cookies.set('token', data.token, { expires: 60 * 60 * 24 * 14 })
            }).catch(({ response }) => {
    ...
    return <div className={c.main}>
        <form className={c.form} onSubmit={handleSubmit}>
            <h1 className={c.title}>Create Seller Account</h1>
            ...
            <div className={c.inp_con}>
                <label className={c.label} htmlFor='rePassword'>Re-enter Password</label>
                <input className={errors.rePassword && touched.rePassword ? c.inp_error : c.inp} id='rePassword' name='rePassword' type='password' placeholder='Re-enter Password' onChange={handleChange} onBlur={handleBlur} value={values.rePassword} />
                {errors.rePassword && touched.rePassword && <span className={c.error}>{errors.rePassword}</span>}
            </div>

            <div className={c.inp_con}>
                <label className={c.label} htmlFor='address'>Add Your Adress</label>
                <input className={errors.address && touched.address ? c.inp_error : c.inp} style={{ marginBottom: errors.address && touched.address ? 45 : 10 }} id='address' name='address' placeholder='USA' onChange={handleChange} onBlur={handleBlur} value={values.address} />
